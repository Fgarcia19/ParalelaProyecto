...
void TSPRec(double adj[N][N], double curr_bound, double curr_weight,
			int level, int curr_path[],bool visited[],int to)
{
...
void TSP(double adj[N][N],int from, int to)
{
	...
	curr_path[0] = from;
		...
		visited[i]=true;
		curr_path[1] = i;
		TSPRec(adj,curr_bound,adj[0][i],2,curr_path,visited,to);
			curr_bound = temp;
		visited[i]=false;
